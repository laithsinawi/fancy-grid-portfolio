...
class acf_upgrade {
	...
	*/
	...
	function admin_menu() {
		...
		// update admin page
		add_submenu_page( 'edit.php?post_type=acf', __( 'Upgrade', 'acf' ), __( 'Upgrade', 'acf' ), 'manage_options', 'acf-upgrade', array(
			$this,
			'html'
		) );
	}
							...
							if ( empty( $sub_fields ) ) {
								...
								foreach ( $sub_fields as $sub_field ) {
									...
									if ( ! isset( $sub_field['key'] ) ) {
										$sub_field['key'] = 'field_' . $sub_field['id'];
									}
			...
			case '3.0.0 (step 4)':
				...
				if ( $values ) {
					...
					foreach ( $values as $value ) {
						...
						if ( $row > $current_row ) {
							$rows[ $value['post_id'] ][ $value['field_id'] ] = (int) $row;
						}
